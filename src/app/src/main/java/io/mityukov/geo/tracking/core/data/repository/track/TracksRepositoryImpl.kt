...
import io.mityukov.geo.tracking.core.data.repository.track.capture.LocalTrackCaptureStatus
...
class TracksRepositoryImpl @Inject constructor(
    ...
    override suspend fun insertTrackAction(action: TrackActionType) =
        withContext(coroutineDispatcher) {
            val trackFile = File(tracksDirectory, tempTrackFileName)

            if (action == TrackActionType.Start) {
                trackFile.writeText("")
            } else if (action == TrackActionType.Stop) {
                val currentStatus = trackCaptureStatusRepository.status.first()
                val trackInProgress =
                    (currentStatus as LocalTrackCaptureStatus.Enabled).trackInProgress
